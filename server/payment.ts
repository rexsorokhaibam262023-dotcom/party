...
export interface CreateOrderParams {
  ...
  amount?: number; // Optional on input, server strictly overrides with EVENT_TICKET_PRICE
  currency?: string;
  customerName: string;
...
export class PaymentService {
  ...
  constructor() {
    ...
    // Server-enforced ticket price (Requirement 6: Never trust client amount)
    const envPrice = process.env.EVENT_TICKET_PRICE ? parseInt(process.env.EVENT_TICKET_PRICE, 10) : 350;
    this.ticketPrice = isNaN(envPrice) || envPrice <= 0 ? 350 : envPrice;
  ...
  async verifyCheckoutPayment(params: {
    ...
    if (this.razorpayClient) {
      try {
        ...
        // Requirement 6: Verify payment amount against server-configured ticket price
        const expectedPaise = this.getEventTicketPrice() * 100;
        const actualPaise = Number(payment.amount);
        if (actualPaise !== expectedPaise) {
          return {
            verified: false,
            error: `Payment amount mismatch: Expected ₹${this.getEventTicketPrice()} (${expectedPaise} paise), but received ₹${actualPaise / 100} (${actualPaise} paise). Ticket generation rejected.`,
          };
        }
